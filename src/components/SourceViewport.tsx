  ...
  },
  {
    ...
    card: {
      ...
      metadata: [
        ...
        { label: "Fields extracted", value: "Site NPIs · PI names · Sponsor · Phase · Eligibility biomarkers · Enrollment status" },
        { label: "Joins to", value: "NPPES (PI NPI) · PubMed (PI publications) · Open Payments (sponsor → PI flows)" },
      ],
      bullets: [
      ...
      {active.format === "pdf-local" && active.pdfPath ? (
        <div className="bg-slate-100">
          ...
          <div className="px-4 py-2.5 text-[10px] text-slate-500 italic">
            {active.blurb} <span className="text-slate-400">· Hosted same-origin in this app for reliable embedding.</span>
          </div>
        </div>
...
function SourceCard({ card, blurb }: { card: CardPreview; blurb: string }) {
  return (
    <div className="bg-gradient-to-br from-slate-50 to-white p-6">
      ...
      <div className="text-center text-[10px] text-slate-400 italic mt-4 max-w-2xl mx-auto">
        {blurb}
      </div>
    </div>
  );
